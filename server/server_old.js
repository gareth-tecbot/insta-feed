  ...
  try {
    ...
    const posts = await page.evaluate(() => {
      ...
      for (let i = 0; i < Math.min(postElements.length, 12); i++) {
        ...
        // Find the image within the post
        const imgElement = postElement.querySelector('img');
        if (imgElement) {
          postsData.push({
            imageUrl: imgElement.src,
            postUrl: postUrl,
            timestamp: new Date().toISOString(), // Instagram doesn't show timestamps on profile grid
            caption: '' // Captions aren't visible in profile grid
          });
        }
      }
