...
async function scrapeInstagramPosts(username, password, sessionId = null) {
  ...
  try {
    // Launch browser with better Instagram compatibility
    ...
    // Set mobile user agent (Instagram serves different HTML to mobile)
    ...
    if (sessionId && userSessions.has(sessionId)) {
      ...
      // Give Instagram a moment to render potential popups
await page.waitForTimeout(2000);

// Close dismissible login popup if it appears
const loginPopupCloseBtn = await page.$('button:has-text("Not Now"), svg[aria-label="Close"]');
if (loginPopupCloseBtn) {
  await loginPopupCloseBtn.click();
          ...
          posts = await page.evaluate((sel) => {
            ...
            for (let i = 0; i < postElements.length && postsData.length < 12; i++) {
              ...
              if (postUrl && !seenUrls.has(postUrl)) {
                ...
                if (imgElement && imgElement.src && imgElement.src !== 'data:image/svg+xml;base64,') {
                  postsData.push({
